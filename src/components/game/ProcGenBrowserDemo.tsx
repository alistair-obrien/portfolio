...
function drawMapTexture(context: CanvasRenderingContext2D, map: EngineMap) {
  const tileSize = getTileSize(map);
  const cssWidth = map.width * tileSize;
  const cssHeight = map.height * tileSize;
  const mask = toMaskImage(map);
  const ambientGradient = context.createLinearGradient(0, 0, cssWidth, cssHeight);
  ambientGradient.addColorStop(0, "#2b2f39");
  ambientGradient.addColorStop(1, "#1f2430");
  const wallLift = Math.max(1, Math.round(tileSize * 0.18));
  ...
  for (let y = 0; y < map.height; y += 1) {
    for (let x = 0; x < map.width; x += 1) {
      const px = x * tileSize;
      const py = y * tileSize;
      const wall = channelAt(mask, x, y, 0) > 0;
      const prop = channelAt(mask, x, y, 1) > 0;
      const item = channelAt(mask, x, y, 2) > 0;
      const character = channelAt(mask, x, y, 3) > 0;
      const noise = hashCell(x, y);
      const northWall = isChannelFilled(mask, x, y - 1, 0);
      const southWall = isChannelFilled(mask, x, y + 1, 0);
      const westWall = isChannelFilled(mask, x - 1, y, 0);
      const eastWall = isChannelFilled(mask, x + 1, y, 0);

      if (wall) {
        if (!southWall || !eastWall) {
          context.fillStyle = "rgba(8, 10, 14, 0.38)";
          context.fillRect(px + wallLift, py + wallLift, tileSize, tileSize);
        }

        context.fillStyle = "#f28c38";
        context.fillRect(px, py, tileSize, tileSize);

        context.fillStyle = "rgba(255, 220, 184, 0.35)";
        context.fillRect(px, py, tileSize, Math.max(1, Math.round(tileSize * 0.18)));

        context.fillStyle = "rgba(140, 59, 20, 0.28)";
        context.fillRect(
          px + tileSize - Math.max(1, Math.round(tileSize * 0.16)),
          py,
          Math.max(1, Math.round(tileSize * 0.16)),
          tileSize,
        );

        context.fillStyle = "rgba(125, 49, 15, 0.36)";
        context.fillRect(
          px,
          py + tileSize - Math.max(1, Math.round(tileSize * 0.2)),
          tileSize,
          Math.max(1, Math.round(tileSize * 0.2)),
        );
        continue;
      }

      const floorShade = 52 - Math.round(y / Math.max(1, map.height) * 8) - Math.round(noise * 4);
      const ambientOcclusion =
        (northWall ? 10 : 0) +
        (westWall ? 6 : 0) +
        (southWall ? 3 : 0) +
        (eastWall ? 2 : 0);
      const checker = (x + y) % 2 === 0 ? 2 : -1;
      const red = floorShade - ambientOcclusion + checker;
      const green = floorShade + 4 - ambientOcclusion + checker;
      const blue = floorShade + 10 - ambientOcclusion + checker;
      context.fillStyle = `rgb(${red}, ${green}, ${blue})`;
      context.fillRect(px, py, tileSize, tileSize);

      context.fillStyle = `rgba(255, 255, 255, ${0.015 + noise * 0.008})`;
      context.fillRect(px, py, tileSize, Math.max(1, tileSize * 0.1));

      if (northWall) {
        context.fillStyle = "rgba(0, 0, 0, 0.12)";
        context.fillRect(px, py, tileSize, Math.max(1, tileSize * 0.12));
      }

      if (westWall) {
        context.fillStyle = "rgba(0, 0, 0, 0.08)";
        context.fillRect(px, py, Math.max(1, tileSize * 0.08), tileSize);
      }

      if (prop) {
        context.fillStyle = "rgba(0, 0, 0, 0.18)";
        context.fillRect(px + tileSize * 0.22, py + tileSize * 0.26, tileSize * 0.52, tileSize * 0.42);
        context.fillStyle = "#5aa4b8";
        context.fillRect(px + tileSize * 0.18, py + tileSize * 0.2, tileSize * 0.56, tileSize * 0.42);
        context.fillStyle = "rgba(234, 249, 252, 0.24)";
        context.fillRect(px + tileSize * 0.22, py + tileSize * 0.24, tileSize * 0.48, tileSize * 0.1);
      }

      if (item) {
        context.fillStyle = "rgba(94, 69, 8, 0.14)";
        context.beginPath();
        context.arc(px + tileSize / 2, py + tileSize / 2, tileSize * 0.24, 0, Math.PI * 2);
        context.fill();
        context.fillStyle = "#f4c94d";
        context.beginPath();
        context.arc(px + tileSize / 2, py + tileSize / 2, tileSize * 0.18, 0, Math.PI * 2);
        context.fill();
      }

      if (character) {
        context.fillStyle = "#d8496a";
        context.beginPath();
        context.arc(px + tileSize / 2, py + tileSize / 2, Math.max(1.5, tileSize * 0.18), 0, Math.PI * 2);
        context.fill();
        context.fillStyle = "rgba(255, 227, 235, 0.75)";
        context.beginPath();
        context.arc(px + tileSize / 2, py + tileSize * 0.42, Math.max(1, tileSize * 0.06), 0, Math.PI * 2);
        context.fill();
      }
    }
  }

  context.strokeStyle = "rgba(242, 140, 56, 0.09)";
  context.lineWidth = 1;
  ...
  context.strokeStyle = "rgba(242, 140, 56, 0.16)";
  context.lineWidth = 2;
